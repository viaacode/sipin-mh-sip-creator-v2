...
def get_mh_mapping(sip: SIP) -> dict:
    ...
    mapping = {
        "Descriptive": {
            ...
            "Description": get_nl_string(ie.description.root) if ie.description else None,
            },
        "Dynamic": {
            "dc_title": get_nl_string(ie.name.root),
            "dc_description": get_nl_string(ie.description.root) if ie.description else None,
            "dc_description_lang": get_nl_string(ie.description.root) if ie.description else None,
            "dcterms_created": ie.date_created.value,
            "dcterms_issued": ie.date_published.value if ie.date_published else None,
            "dc_rights_rightsOwners": [("Auteursrechthouder", get_nl_string(owner.name.root)) for owner in ie.copyright_holder],
            "ContentCategory": "image",
            "dc_subjects": [("Trefwoord", trefwoord) for trefwoord in get_nl_strings(ie.keywords.root)] if ie.keywords else None,
            "dc_identifier_localid": ie.local_identifier[0].value if len(ie.local_identifier) else None,
            "dc_identifier_localids": [("local_id", local_id.value) for local_id in ie.local_identifier],
            "dc_languages": [("multiselect", lang) for lang in ie.in_language],
            "dc_titles": [("title", title.id) for title in ie.is_part_of],
            "dc_creators": [(creator.role_name, get_nl_string(creator.creator.name.root)) for creator in ie.creator if creator.creator],
            "dc_contributors": [(contributor.role_name, get_nl_string(contributor.contributor.name.root)) for contributor in ie.contributor if contributor.contributor],
            "dc_publishers": [(publisher.role_name, get_nl_string(publisher.publisher.name.root)) for publisher in ie.publisher if publisher.publisher],
            "dc_types": [("multiselect", genre) for genre in get_nl_strings(ie.genre.root)] if ie.genre else None,
            "dc_coverages": [("ruimte", get_nl_string(ruimte.name.root)) for ruimte in ie.spatial] + [("tijd", tijd) for tijd in get_nl_strings(ie.temporal.root)] if ie.temporal else [],
            ...
            "dc_rights_credit": get_nl_string(ie.credit_text.root) if ie.credit_text else None,
...
def get_licenses(sip: SIP) -> list[tuple[str, str]]:
    ...
    return [("multiselect", get_nl_string(license.pref_label.root)) for license in sip.entity.license if type(license) is Concept]
